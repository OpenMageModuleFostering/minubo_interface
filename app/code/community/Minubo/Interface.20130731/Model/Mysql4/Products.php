...
class Minubo_Interface_Model_Mysql4_Products extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function loadAll(){
        ...
        $where = $this->_getReadAdapter()->quoteInto("entity_id > ?", 0);
				...
				$select = $this->_getReadAdapter()->select()->from($table,array('entity_id','name','sku','sku_type','computer_manufacturers','computer_manufacturers_value','country_orgin','color','color_value','price','tax_class_id','cost','msrp','msrp_display_actual_price_type','msrp_enabled','shoe_size','shoe_size_value'))->where($where)->order('entity_id');
